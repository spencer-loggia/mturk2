...
def train(cfg: TrainConfig):
    agent, deform = QAgent(cfg).to(cfg.device), Deform(channels=cfg.percept_dim, deform_basis=4, groups=cfg.dims)
...
def train_behavior(cfg: TrainConfig, csv_paths: Sequence[str]):
    ...
    tau = 1. #torch.nn.Parameter(torch.tensor(cfg.temperature, device=cfg.device))
    ...
    for epoch in range(1, cfg.epochs + 1):
        TB = T * B
        oa_tbk2 = angles_tbk2.reshape(TB, k, 2)
        ...
        loss = ce + deform.compute_penalty()
        ...
        if epoch % 10 == 0:
            print(f"Ep {epoch:04d} | CE {ce.item():.3f} | Acc {acc:.2%}")
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
        ...
        percept_dim=4, reward_dim=1, action_dims=(1,), device="cpu",
        headdim=9, chunk_size=256,
        batch_size=4, epochs=3000, eval_interval=100,
        lr=2e-2, gamma=0.0, dims=2, max_trials=256*150, num_choices=1,
        seed_override=None, run_name="cogssm",
        input_dim=4, output_dim=1,
    )
    # train(cfg)
    # For behavior learning:
    train_behavior(cfg, ["/Users/loggiasr/Projects/colorcontigency2/mturk2/mturk2_code/saved_data/all_Yuri.csv",
                                  "/Users/loggiasr/Projects/colorcontigency2/mturk2/mturk2_code/saved_data/all_Sally.csv",
                                  "/Users/loggiasr/Projects/colorcontigency2/mturk2/mturk2_code/saved_data/all_Buzz.csv",
                                  "/Users/loggiasr/Projects/colorcontigency2/mturk2/mturk2_code/saved_data/all_Tina.csv"])